...
const LoginPage = () => {
  ...
  const handleSignIn = async (e) => {
    ...
    try {
      ...
      setDisplayError(formatErrorMessage(error.message || error));
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      // The signInWithGoogle function now uses popup for both localhost and production
  ...
  // Function to format error messages professionally
  const formatErrorMessage = (error) => {
    if (typeof error === 'string') {
      // Handle error messages that are already strings
      if (error.includes('Firebase: Error (auth/invalid-credential)') || 
      ...
      if (error.includes('Firebase: Error (auth/too-many-requests)') || 
      ...
      if (error.includes('Firebase: Error') || error.includes('auth/')) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-blue-900 flex flex-col items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative z-10 max-w-md w-full space-y-8 bg-white/10 backdrop-blur-lg p-10 rounded-3xl shadow-2xl border border-white/20">
          ...
          {displayError && (
            <div className="mt-6 bg-gradient-to-r from-red-500/5 to-pink-500/5 border border-red-400/30 p-5 rounded-2xl shadow-lg animate-fadeIn backdrop-blur-sm" role="alert">
              <div className="flex items-start space-x-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-red-200 font-semibold text-sm mb-2 tracking-wide">Authentication Failed</h3>
                  <p className="text-red-300/90 text-sm leading-relaxed">{displayError}</p>
                </div>
              </div>
            </div>
          )}
      ...
      <div className="relative z-10 max-w-md w-full bg-white/10 backdrop-blur-lg p-10 rounded-3xl shadow-2xl border border-white/20">
        ...
        {activeTab === 'signin' ? (
          <form className="mt-8 space-y-6" onSubmit={handleSignIn}>
            ...
            {/* Google Sign-In Button - Slightly Larger */}
            <button
              type="button"
              onClick={handleGoogleSignIn}
              disabled={loading}
              className="group relative w-full flex items-center justify-center py-3 px-6 border border-white/20 text-sm font-medium rounded-xl text-white bg-white/10 backdrop-blur-sm hover:bg-white/20 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-white/50 shadow-lg transform transition-all duration-150 hover:shadow-xl hover:-translate-y-0.5"
            >
              {loading ? (
                <span className="flex items-center">
                  <svg className="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 7 14 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                  </svg>
                </span>
              ) : (
