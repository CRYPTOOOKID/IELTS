...
const SentenceBuilderGame = ({ onBackToGames }) => {
  ...
  const loadSentences = async (selectedDifficulty) => {
    setIsLoading(true);
    setLoadingMessage('🧠 AI is crafting fresh questions...');
    ...
    try {
      ...
      if (generatedSentences.length > 0) {
        ...
        setLoadingMessage('✨ Perfect! Setting up your game...');
        console.log('Generated sentences:', generatedSentences);
      } else {
        setLoadingMessage('📚 Using curated questions...');
      ...
      setLoadingMessage('📚 Loading backup questions...');
            ...
            {isCorrect !== null && (
              ...
              >
                {isCorrect ? '🎉 Perfect! Excellent work!' : 
                 attempts >= maxAttempts ? '❌ Incorrect! Here\'s the answer:' : '🤔 Not quite right, try again!'}
              </motion.div>
            )}
          </AnimatePresence>
